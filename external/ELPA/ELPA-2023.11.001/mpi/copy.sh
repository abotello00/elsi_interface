...
ELPA_ROOT=/scratch/ukh0001/elpa-2023.11.001/
...
elpa_dir=$ELPA_ROOT/src/ftimings
for file in \
  highwater_mark.c \
  resident_set_size.c \
  time.c \
...
elpa_dir=$ELPA_ROOT/build_generic
elpa_dir_avx=$ELPA_ROOT/build_avx
elpa_dir_avx2=$ELPA_ROOT/build_avx2
elpa_dir_avx512=$ELPA_ROOT/build_avx512
elpa_dir_cuda=$ELPA_ROOT/build_cuda
...
# elpa_constants.f90
origin_file=`ls $elpa_dir/*elpa_constants* | tail -n 1`
cp $origin_file elpa_constants.f90

origin_file=`ls $elpa_dir_avx/*elpa_constants* | tail -n 1`
cp $origin_file elpa_constants_avx.f90

origin_file=`ls $elpa_dir_avx2/*elpa_constants* | tail -n 1`
cp $origin_file elpa_constants_avx2.f90

origin_file=`ls $elpa_dir_avx512/*elpa_constants* | tail -n 1`
cp $origin_file elpa_constants_avx512.f90

origin_file=`ls $elpa_dir_cuda/*elpa_constants* | tail -n 1`
cp $origin_file elpa_constants_cuda.f90


origin_file=`ls $elpa_dir_cuda/*mod_cuda* | tail -n 1`
cp $origin_file mod_cuda.f90

# config-f90.h is not needed
cp $elpa_dir_cuda/config-f90.h .

elpa_dir_cuda_src=$ELPA_ROOT/src/GPU/CUDA
cp $elpa_dir_cuda_src/*.cu .

elpa_dir_elpa=$ELPA_ROOT/build_generic/elpa
cp -r $elpa_dir_elpa .
