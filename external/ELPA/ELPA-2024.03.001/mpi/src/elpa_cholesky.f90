  ...
  do n = 1, na, nblk
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (useGPU .and. .not. useCCL) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1_dev to tmp1", 873,  successGPU)
      ...
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1 to tmp1_dev", 966,  successGPU)
      endif
    ...
    if (useGPU) then
      if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(int(loc(tmatc),kind=c_intptr_t), tmatc_dev, num, &
                              gpuMemcpyDeviceToHost)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc_dev to tmatc", 1053,  successGPU)
      endif
    endif
    ...
    if (useGPU) then
      !if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(tmatc_dev, int(loc(tmatc),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc to tmatc_dev", 1182,  successGPU)
      !endif
    endif ! useGPU
