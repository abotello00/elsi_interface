  ...
  deallocate(ibreq)
    ...
    if (l_rows > 0) then
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("trans_ev_band_to_full: tmat_complete -> tmat_complete_dev", 1425,  successGPU)
        ...
        if (src < my_prow) then
          ...
          if (useGPU) then
            ...
            if (allComputeOnGPU) then
              ...
              call check_memcpy_GPU_f("tridi_to_band: row_dev -> row_group_dev", 1097,  successGPU)

              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 1103,  successGPU)
          ...
          if (useGPU) then
            ...
            if (allComputeOnGPU) then
              ...
              successGPU =  gpu_memcpy(c_loc(row_group_mpi_fortran_ptr(1,row_group_size)), &
                                       ...
                                       l_nev* size_of_datatype,      &
                                       gpuMemcpyDeviceToDevice)
              call check_memcpy_GPU_f("tridi_to_band: row_dev -> row_group_dev", 1464,  successGPU)

              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 1470,  successGPU)
              if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
            else
              row_group(1:l_nev,row_group_size) = row(1:l_nev)
            endif
    ...
    if (current_local_n > 1) then
      ...
      if (useGPU .and. .not.(allComputeOnGPU)) then
        ...
        successGPU =  gpu_memcpy(bcast_buffer_dev, int(loc(bcast_buffer(1,1)),kind=c_intptr_t),  &
                                 ...
                                 size_of_datatype, &
                                 gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("tridi_to_band: bcast_buffer -> bcast_buffer_dev", 2148,  successGPU)
                ...
                if (allComputeOnGPU) then
                  if (next_top_msg_length > 0) then
                    successGPU =  gpu_memcpy(c_loc(top_border_recv_buffer_mpi_fortran_ptr(1,i)), &
                                             ...
                                             gpuMemcpyDeviceToDevice)
                    call check_memcpy_GPU_f("tridi_to_band: bottom_border_send_dev -> top_border_recv_dev", 3041,  successGPU)
                  endif

              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 3048,  successGPU)
              if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
                else ! allComputeOnGPU
                  if (next_top_msg_length > 0) then
                ...
                if (allComputeOnGPU) then
                  if (next_top_msg_length > 0) then
                    successGPU =  gpu_memcpy(c_loc(top_border_recv_buffer_mpi_fortran_ptr(1,i)), &
                                             ...
                                             stripe_width*next_top_msg_length* size_of_datatype,      &
                                             gpuMemcpyDeviceToDevice)
                    call check_memcpy_GPU_f("tridi_to_band: bottom_border_send_dev -> top_border_recv_dev", 3416,  successGPU)
                  endif

              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 3423,  successGPU)
              if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
                else ! allComputeOnGPU
                  if (next_top_msg_length > 0) then
                    top_border_recv_buffer(1:stripe_width*next_top_msg_length,i) =  &
               ...
               if (sweep==0 .and. current_n_end < current_n .and. l_nev > 0) then
                 successGPU =  gpu_memcpy(c_loc(bottom_border_recv_buffer_mpi_fortran_ptr(1,i)), &
                                          ...
                                          nbw*stripe_width* size_of_datatype,      &
                                           gpuMemcpyDeviceToDevice)
                 call check_memcpy_GPU_f("tridi_to_band: top_border_send_dev -> bottom_border_recv_dev", 4154,  successGPU)
               endif
              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 4160,  successGPU)
              if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
               if (next_n_end < next_n) then
                 successGPU =  gpu_memcpy(c_loc(bottom_border_recv_buffer_mpi_fortran_ptr(1,i)), &
                                          c_loc(top_border_send_buffer_mpi_fortran_ptr(1,i)),  &
                                          nbw*stripe_width* size_of_datatype,      &
                                           gpuMemcpyDeviceToDevice)
                 call check_memcpy_GPU_f("tridi_to_band: top_border_send_dev -> bottom_border_recv_dev", 4183,  successGPU)
               endif
              if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
              successGPU = gpu_devicesynchronize()
              call check_memcpy_GPU_f("tridi_to_band: device_synchronize", 4189,  successGPU)
              if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
             else ! allComputeOnGPU
               if (sweep==0 .and. current_n_end < current_n .and. l_nev > 0) then
