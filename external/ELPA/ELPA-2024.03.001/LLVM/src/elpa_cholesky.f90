...
!
  ...
  if (useGPU) then
    ...
    call check_alloc_GPU_f("elpa_cholesky: tmp1_dev", 271,  successGPU)

    successGPU = gpu_memset(tmp1_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmp1_dev", 282,  successGPU)

    successGPU = gpu_malloc(tmp2_dev, nblk*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmp2_dev", 286,  successGPU)

    successGPU = gpu_memset(tmp2_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmp2_dev", 297,  successGPU)

    successGPU = gpu_malloc(tmatc_dev, l_cols*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmatc_dev", 301,  successGPU)

    successGPU = gpu_memset(tmatc_dev, 0, l_cols*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmatc_dev", 312,  successGPU)

    successGPU = gpu_malloc(tmatr_dev, l_rows*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmatr_dev", 316,  successGPU)

    successGPU = gpu_memset(tmatr_dev, 0, l_rows*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmatr_dev", 327,  successGPU)

    successGPU = gpu_malloc(a_dev, matrixRows*matrixCols*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: a_dev", 332,  successGPU)
  endif ! useGPU

  allocate(tmp1(nblk*nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmp1", 353,  istat,  errorMessage)

  allocate(tmp2(nblk,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmp2", 356,  istat,  errorMessage)
  ...
  call check_allocate_f("elpa_cholesky: tmatr", 372,  istat,  errorMessage)

  allocate(tmatc(l_cols,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmatc", 375,  istat,  errorMessage)
  ...
  if (useGPU) then
    ...
    call check_memcpy_GPU_f("elpa_cholesky 1: memcpy a-> a_dev", 413,  successGPU)
  endif
    ...
    if (n+nblk > na) then
      ...
      if (useGPU) then
        if (my_prow==prow(n, nblk, np_rows) .and. my_pcol==pcol(n, nblk, np_cols)) then
          ...
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 467,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 490,  successGPU)
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          ...
          successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 638,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 661,  successGPU)
          call obj%timer%stop("blas")
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1_dev to tmp1", 808,  successGPU)
      endif
!#endif
...
!      call check_memcpy_GPU_f("cholesky: device_synchronize", 832,  successGPU)
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1 to tmp1_dev", 868,  successGPU)
      endif
!#endif
    ...
    if (useGPU) then
      if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(int(loc(tmatc),kind=c_intptr_t), tmatc_dev, num, &
                              gpuMemcpyDeviceToHost)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc_dev to tmatc", 967,  successGPU)
      endif
    endif
!#endif
...
!    if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
!    successGPU = gpu_devicesynchronize()
!    call check_memcpy_GPU_f("cholesky: device_synchronize", 993,  successGPU)
!    if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
!
!    do i=1,nblk
!      call obj%timer%start("mpi_cuda_communication")
    ...
    if (useGPU) then
      !if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(tmatc_dev, int(loc(tmatc),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc to tmatc_dev", 1030,  successGPU)
      !endif
    endif
!#endif
...
!#if !defined(1) || (defined(1) && defined(WITH_CUDA_AWARE_MPI))
      ...
      successGPU = gpu_memcpy(int(loc(tmatr),kind=c_intptr_t), tmatr_dev, num, &
                              gpuMemcpyDeviceToHost)
      call check_memcpy_GPU_f("elpa_cholesky: tmatr_dev to tmatr", 1087,  successGPU)
    endif
    ...
    if (useGPU) then
      ...
      successGPU = gpu_memcpy(tmatr_dev, int(loc(tmatr),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
      call check_memcpy_GPU_f("elpa_cholesky: tmat to tmatr_dev", 1122,  successGPU)
    endif
  ...
  if (useGPU) then
    ...
    call check_dealloc_GPU_f("elpa_cholesky: tmp1_dev", 1173,  successGPU)

    successGPU = gpu_free(tmp2_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmp1_dev", 1176,  successGPU)

    successGPU = gpu_free(tmatc_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmatc_dev", 1179,  successGPU)

    successGPU = gpu_free(tmatr_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmatr_dev", 1182,  successGPU)
  endif


  deallocate(tmp1, tmp2, tmatr, tmatc, stat=istat, errmsg=errorMessage)
  call check_deallocate_f("elpa_cholesky: tmp1, tmp2, tmatr, tmatc", 1199,  istat,  errorMessage)
  ...
  if (useGPU) then
    successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
                     matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy 2 a-> a_dev", 1223,  successGPU)
  endif
  ...
  if (useGPU) then
    ...
    !successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
    !                   matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    !call check_memcpy_GPU_f("elpa_cholesky: memcpy a-> d_dev", 1272,  successGPU)

    successGPU = gpu_free(a_dev)
    call check_dealloc_GPU_f("elpa_cholesky: a_dev", 1275,  successGPU)
...
!
  ...
  if (useGPU) then
    ...
    call check_allocate_f("elpa_cholesky: a_tmp", 342,  istat,  errorMessage)
  endif ! useGPU
      ...
      if (useGPU) then
        if (my_prow==prow(n, nblk, np_rows) .and. my_pcol==pcol(n, nblk, np_cols)) then
          ...
          successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 526,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 549,  successGPU)
          call obj%timer%stop("blas")
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          ...
          successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 697,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_tmp-> a_dev", 720,  successGPU)
          call obj%timer%stop("blas")
...
!#if !defined(1) || (defined(1) && defined(WITH_CUDA_AWARE_MPI))
  ...
  if (useGPU) then
    successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                     matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy 2 a-> a_dev", 1245,  successGPU)
  endif
  ! Set the lower triangle to 0, it contains garbage (form the above matrix multiplications)
  ...
  !endif ! useGPU
    ...
    successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy a_tmp-> a_dev", 1302,  successGPU)



    deallocate(a_tmp, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("elpa_cholesky: a_tmp", 1312,  istat,  errorMessage)
