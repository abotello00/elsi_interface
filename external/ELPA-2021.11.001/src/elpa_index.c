...
enum ELPA_ERRORS {
        ELPA_OK = 0, ELPA_ERROR = -1, ELPA_ERROR_ENTRY_NOT_FOUND = -2, ELPA_ERROR_ENTRY_INVALID_VALUE = -3, ELPA_ERROR_ENTRY_ALREADY_SET = -4, ELPA_ERROR_ENTRY_NO_STRING_REPRESENTATION = -5, ELPA_ERROR_SETUP = -6, ELPA_ERROR_CRITICAL = -7, ELPA_ERROR_API_VERSION = -8, ELPA_ERROR_AUTOTUNE_API_VERSION = -9, ELPA_ERROR_AUTOTUNE_OBJECT_CHANGED = -10, ELPA_ERROR_ENTRY_READONLY = -11, ELPA_ERROR_CANNOT_OPEN_FILE = -12,
};
...
enum ELPA_AUTOTUNE_PARTS {
 ...
 void elpa_eigenvectors_all_host_arrays_dc(elpa_t handle, double _Complex *a, double *ev, double _Complex *q, int *error);
 void elpa_eigenvectors_all_host_arrays_fc(elpa_t handle, float _Complex *a, float *ev, float _Complex *q, int *error);
 ...
 void elpa_eigenvectors_device_pointer_dc(elpa_t handle, double _Complex *a, double *ev, double _Complex *q, int *error);
 void elpa_eigenvectors_device_pointer_fc(elpa_t handle, float _Complex *a, float *ev, float _Complex *q, int *error);
 ...
 void elpa_eigenvalues_all_host_arrays_dc(elpa_t handle, double _Complex *a, double *ev, int *error);
 void elpa_eigenvalues_all_host_arrays_fc(elpa_t handle, float _Complex *a, float *ev, int *error);
 ...
 void elpa_eigenvalues_device_pointer_dc(elpa_t handle, double _Complex *a, double *ev, int *error);
 void elpa_eigenvalues_device_pointer_fc(elpa_t handle, float _Complex *a, float *ev, int *error);
 ...
 void elpa_hermitian_multiply_dc(elpa_t handle, char uplo_a, char uplo_c, int ncb, double _Complex *a, double _Complex *b, int nrows_b, int ncols_b, double _Complex *c, int nrows_c, int ncols_c, int *error);
 void elpa_hermitian_multiply_fc(elpa_t handle, char uplo_a, char uplo_c, int ncb, float _Complex *a, float _Complex *b, int nrows_b, int ncols_b, float _Complex *c, int nrows_c, int ncols_c, int *error);
 void elpa_cholesky_d(elpa_t handle, double *a, int *error);
 void elpa_cholesky_f(elpa_t handle, float *a, int *error);
 void elpa_cholesky_dc(elpa_t handle, double _Complex *a, int *error);
 void elpa_cholesky_fc(elpa_t handle, float _Complex *a, int *error);
 void elpa_invert_trm_d(elpa_t handle, double *a, int *error);
 void elpa_invert_trm_f(elpa_t handle, float *a, int *error);
 void elpa_invert_trm_dc(elpa_t handle, double _Complex *a, int *error);
 void elpa_invert_trm_fc(elpa_t handle, float _Complex *a, int *error);
...
static const int default_max_stored_rows = 256;   
...
static const elpa_index_int_entry_t int_entries[] = {
        ...
        { . base = { . name = "mpi_comm_rows", . description = "Communicator for inter-row communication", . once = 0, . readonly = 0, . env_default = "ELPA_DEFAULT_" "mpi_comm_rows", . env_force = "ELPA_FORCE_" "mpi_comm_rows", . print_flag = PRINT_NO, }, },
        { . base = { . name = "mpi_comm_cols", . description = "Communicator for inter-column communication", . once = 0, . readonly = 0, . env_default = "ELPA_DEFAULT_" "mpi_comm_cols", . env_force = "ELPA_FORCE_" "mpi_comm_cols", . print_flag = PRINT_NO, }, },
        { . base = { . name = "mpi_comm_parent", . description = "Parent communicator", . once = 0, . readonly = 0, . env_default = "ELPA_DEFAULT_" "mpi_comm_parent", . env_force = "ELPA_FORCE_" "mpi_comm_parent", . print_flag = PRINT_NO, }, },
	...
	{ . base = { . name = "use_gpu_id", . description = "Calling MPI task will use this gpu id", . once = 0, . readonly = 0, . env_default = "ELPA_DEFAULT_" "use_gpu_id", . env_force = "ELPA_FORCE_" "use_gpu_id", . print_flag = PRINT_YES, }, . default_value = -99, . autotune_level_old = ELPA_AUTOTUNE_NOT_TUNABLE, . autotune_level = ELPA_AUTOTUNE_NOT_TUNABLE, . autotune_domain = ELPA_AUTOTUNE_DOMAIN_ANY, . autotune_part = ELPA_AUTOTUNE_PART_NONE, . cardinality = use_gpu_id_cardinality, . enumerate = use_gpu_id_enumerate, . valid = use_gpu_id_is_valid, . to_string = ((void*)0), },
        ...
        { . base = { . name = "max_stored_rows", . description = "Maximum number of stored rows used in ELPA 1 backtransformation", . once = 0, . readonly = 0, . env_default = "ELPA_DEFAULT_" "max_stored_rows", . env_force = "ELPA_FORCE_" "max_stored_rows", . print_flag = PRINT_YES, }, . default_value = 256, . autotune_level_old = ELPA_AUTOTUNE_EXTENSIVE, . autotune_level = ELPA1_AUTOTUNE_MAX_STORED_ROWS, . autotune_domain = ELPA_AUTOTUNE_DOMAIN_ANY, . autotune_part = ELPA_AUTOTUNE_PART_ELPA1, . cardinality = max_stored_rows_cardinality, . enumerate = max_stored_rows_enumerate, . valid = max_stored_rows_is_valid, . to_string = ((void*)0), },
